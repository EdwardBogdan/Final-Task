...
#endif
        protected static void InitializeSystem(string group, string addressableKey)
        {
           SystemInitializer.AddToQueueSystem(group, addressableKey, OnInitCompleted);
        }

        private static void OnInitCompleted(ScriptableObject SO, Transform group)
        {
            I = (T)SO;

            string name = I.linkPrefab.gameObject.name;

            GameObject obj = Instantiate(I.linkPrefab.gameObject, group);
